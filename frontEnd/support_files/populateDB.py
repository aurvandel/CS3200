...
db=sqlite3.connect('namesdb.db')
with open('2018all.json', encoding='utf-8-sig') as json_file:
...
#Time to generate the create and insert queries and apply it to the sqlite3 database       
    create_query = "create table if not exists namesT ({0})".format(" text,".join(columns))
    insert_query = "insert into namesT ({0}) values (?{1})".format(",".join(columns), ",?" * (len(columns)-1))    
